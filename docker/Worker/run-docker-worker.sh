...
export model_path='/platform_tech/xiongrongkang/checkpoint/qinshu/qinshu_extract_all_v4_qwen1_5_4b'

sudo docker run --rm -d --security-opt seccomp:unconfined -e OPENBLAS_NUM_THREADS=1 \
    --gpus '"device=7"' -e NUM_GPUS=1 \
    -e TZ=Asia/Shanghai \
    -e NVIDIA_DRIVER_CAPABILITIES="compute,utility" \
    --cpus="10" --memory="20g" --shm-size="8gb" \
    -e FASTCHAT_CONTROLLER_ADDRESS='192.168.80.34' \
    ...
    --network="host" \
    --name ${docker_name} fastchat-worker:0.1